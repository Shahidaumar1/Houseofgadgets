...
class ProductSpecs extends Component
{
        ...
        if ($property == 'selectedMemorySize' || $property == 'selectedGrade' || $property == 'selectedColor') {
            ...
            if ($this->selectedMemorySize || $this->selectedColor && $this->selectedGrade) {
                if ($this->selectedColor && $this->selectedGrade) {
                    $product_spec = $query->where('memory_size', $this->selectedMemorySize)->where('grade', $this->selectedGrade)->where('color', $this->selectedColor)->first();
                } elseif ($this->selectedMemorySize && $this->selectedGrade) {
                    $product_spec = $query->where('memory_size', $this->selectedMemorySize)->where('grade', $this->selectedGrade)->first();
                } else {
                    $product_spec = $query->where('memory_size', $this->selectedMemorySize)->where('grade', $this->selectedGrade)->where('color', $this->selectedColor)->first();
                }
                if ($product_spec) {
    ...
    public function initFirstSpec()
    {
        ...
        if (in_array(!null, $this->available_grades)) {
            ...
            $this->available_colors = $query->where('grade', $this->selectedGrade)->distinct('color')->pluck('color')->toArray();
        }
