...
<head>
    ...
    {{-- THEME (server-rendered CSS) --}}
    <link rel="stylesheet" href="{{ route('asset.theme', ['v' => cache('site.theme_version', 1)]) }}">

    {{-- small bootstrap-danger overrides you wanted --}}
    <style>
    ...
    {{-- NOTE: Removed the localStorage-based theme override script.
         Now theme comes only from server CSS so it applies to ALL users consistently. --}}
</head>

<body>
    {{-- ═══════════════════════════════════════════════════
         SiteSetting — SIRF EK BAAR (cookie + whatsapp dono)
    ════════════════════════════════════════════════════ --}}
    @php
    ...
    {{-- ══════════════════════════════════════
         MAIN COOKIE POPUP — Red Brand Color
    ══════════════════════════════════════ --}}
    ...
    ">
        ...
        ">
            ...
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;position:relative;">
                ...
                <div>
                    <p style="margin:0;color:#fff;font-size:15px;font-weight:700;line-height:1.2;">{{ $cookieBizName }}</p>
                    <p style="margin:0;color:rgba(255,255,255,0.55);font-size:10px;">{{ $cookieDomain }}</p>
                </div>
        ...
        <div style="background:#fff;padding:1.25rem 1.75rem;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                ...
                <a href="{{ rtrim($cookieWebsiteUrl,'/') }}/privacy-policy"
    ...
    ">
        ...
        <div style="background:linear-gradient(135deg, #85898c, #85898c);padding:1.25rem 1.75rem;display:flex;align-items:center;justify-content:space-between;">
            <div>
                ...
                <p style="margin:0;color:rgba(255,255,255,0.6);font-size:11px;">{{ $cookieDomain }}</p>
            </div>
...
@if(!empty($setting) && !empty($setting->whatsapp_number))
    @php
